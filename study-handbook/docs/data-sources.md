...
- 当前核对范围：全册 10 个单元的目录、主题词和核心句型；Unit 1 第 7-14 页及第 87-89 页 `Words to use` 总表已逐页核对。
- 用途：暑假预习的单词、默写和句型练习。
...
当前“朗读”优先在线查询 Free Dictionary API 的英文发音链接，浏览器直接播放远程 MP3，不下载或提交音频文件。词组没有整句音源时，页面会自动拆成单词依次播放在线音源；对于词典未覆盖但已核验的词，使用 Wikimedia Commons 的公开发音文件。接口响应或 Commons 文件页会提供原始链接和许可证，页面会显示该链接。只有查询不到任何在线音源时，才回退到设备浏览器的英文语音合成。两者都不是教材官方录音。教材官方音频在获得可公开使用的音频文件或官方授权链接后，再以单词的 `audioUrl` 字段接入；不要把来源不明的音频上传到仓库。
