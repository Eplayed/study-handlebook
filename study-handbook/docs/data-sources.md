...
当前“朗读”优先在线查询 Free Dictionary API 的英文发音链接，浏览器直接播放远程 MP3，不下载或提交音频文件。接口响应会提供 Wikimedia Commons 原始链接和许可证，页面会显示该链接；查询不到或远程音频播放失败时，才回退到设备浏览器的英文语音合成。两者都不是教材官方录音。教材官方音频在获得可公开使用的音频文件或官方授权链接后，再以单词的 `audioUrl` 字段接入；不要把来源不明的音频上传到仓库。
