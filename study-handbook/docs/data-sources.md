...
- 当前核对范围：全册 10 个单元的目录、主题词和核心句型；第 87-89 页 `Words to use` 总表及每个单元的 `Talking time` / `Find the rule` 句型页已逐页核对。应用内单词下方例句用于帮助理解，不作为教材原文。
- 用途：暑假预习的单词、默写和句型练习。
...
当前“朗读”使用国内在线 MP3 接口，不下载或提交音频文件：单词和短词组使用有道词典接口，完整句型使用百度翻译语音接口。页面会分别标记“国内快速音源”和“国内句型音源”，并链接至来源站点；两者都不是教材官方录音，也不是开源音频。在线音源无法播放时，才回退到设备浏览器的英文语音合成。教材官方音频在获得可公开使用的音频文件或官方授权链接后，再以单词的 `audioUrl` 字段接入；不要把来源不明的音频上传到仓库。
